...
if nargin > 5
    ...
    if outerIter > k
        ...
        if ~isinf(printEvery) && ~mod(k,printEvery)
            printFcn(k,history);
            % save(strcat('./output/net-group-bind/net-', saveName(k, 4), '.mat'), 'x');
        end
        k = outerIter;
    end
...
fprintf('Iter %5d, f(x) = %f, psnr = %f, ||grad||_infty = %f', ...
    k, history(k,1)/(255^2), 10*log10(180^2/(2*history(k,1)/(255^2))), history(k,2) );
